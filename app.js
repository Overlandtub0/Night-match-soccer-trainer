...
function openSettings() {
  ...
  if (key) loadModels(key).catch(() => {}); // refresh dropdown from the live key
...
async function loadModels(key) {
  ...
  if (!res.ok) throw new Error(data?.error?.message || 'Could not list models for this key.');
...
async function reconcileModel() {
  const key = store.get('apiKey');
  if (!key) return;
  try { await loadModels(key); } catch { /* offline or bad key — surfaced when they ask */ }
}
function refreshKeyStatus() {
  const has = !!store.get('apiKey');
  $('#keyStatus').innerHTML = has
    ? '<span class="dot dot--on"></span><span class="key-status-text">API key active</span>'
    : '<span class="dot dot--off"></span><span class="key-status-text">No API key</span>';
}
...
async function callGemini(systemInstruction, userText, useGrounding = true, jsonMode = false, _retry = false) {
  const key = store.get('apiKey');
  if (!key) throw new Error('Add your Google Gemini API key in Settings first.');
  ...
  if (useGrounding) body.tools = [/gemini-1\.5|gemini-1\.0/.test(model) ? { google_search_retrieval: {} } : { google_search: {} }];
  if (jsonMode) body.generationConfig = { responseMimeType: 'application/json', temperature: 0.7 };
...
async function sendMessage(text) {
  if (!text || busy) return;
  if (!store.get('apiKey')) { openSettings(); toast('Add your free API key to start', false); return; }
...
function initPlans() {
  $('#planForm').addEventListener('submit', async e => {
    e.preventDefault();
    if (!store.get('apiKey')) { openSettings(); toast('Add your free API key first', false); return; }
    const f = e.target;
...
function init() {
  ...
  reconcileModel(); // if a key is set, make sure the stored model is one it can use
}
